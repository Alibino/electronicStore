...
namespace electronicStore
{
    public class DatabaseService : IDatabaseService
    {
        ...
        public void CreateWare(string navn, double pris)
        {
            Ware ware = new Ware(navn, pris);
            _dbContext.Wares.Add(ware);
            _dbContext.SaveChanges();
        }

        public void DeleteAllWares()
        {
            foreach(Ware w in GetAllWares())
            {
                _dbContext.Wares.Remove(w);
            }
            _dbContext.SaveChanges();
        }
